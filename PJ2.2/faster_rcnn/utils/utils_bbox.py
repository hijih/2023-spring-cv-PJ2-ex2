...
class DecodeBox():
    ...
    def get_box(self, box_xy, box_wh, input_shape, image_shape):
        box_yx = box_xy[..., ::-1]
        box_hw = box_wh[..., ::-1]
    ...
    def forward(self, roi_cls_locs, roi_scores, rois, image_shape, input_shape, nms_iou = 0.3, confidence = 0.5,stage=2):
        results = []
        bs      = len(roi_cls_locs)
        ...
        for i in range(bs):
            ...
            if len(results[-1]) > 0:
                ...
                results[-1][:, :4] = self.get_box(box_xy, box_wh, input_shape, image_shape)
